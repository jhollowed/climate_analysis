...
public :: &
   ...
   tracers_timestep_forcing    !--JH--  Adding Aman's clock tracer function
...
!--JH--
! ============ BEGIN ==================
! Aman : This function created from scratch to force the boundary value for
! tracer at every time step
! Function designed to compute only the clock tracer as of now. Hence num_tracer=1
! In the main for loop, thus, boundary values are mentioned for just one tracer;
! the clock tracer
subroutine tracers_timestep_forcing(state, ptend, dt)
    ...
    real              :: t             ! tracer boundary condition
    real(r8)          :: spinup_nday   ! number of days for the spinup period
    !------------------------------------------------------------------
    ...
    ! set tendencies to zero; no update of q, so we can manually adjust later 
    call physics_ptend_init(ptend,state%psetcols, 'tracers', lq=lq)
    
    lchnk = state%lchnk
    ncol = state%ncol
    !--JH-- loop through vertical levels
    do k = 1, pver
        ! For clock tracer
        
        ! modify state of tracer field only for the bottom (?) 2 levels (i.e. the surface?)
        ! Aman: if (k >= pver-2 .and. k <= pver) then
        
        ! --JH--:
        ! pref_mid_norm are layer midpoints normalized by surface pressure
        ! "run" clocks as long as this coordinate is larger than 0.7 (nearer surface)
        if (pref_mid_norm(k) >= 0.7) then
            do i = 1, ncol
                ! passive tracer count begins at ixtrct to
                ! ixtrct + num_tracer -1
                ! --JH-- 
                ! set state mixing ratio to t if simulation time has passed 300
                ! days, else do nothing? --> clock tracers activated only after
                ! day 300?  
                if (t >= spinup_nday) then
    ...
    !--JH-- What name is being assigned here? When/how is the tracers_suite made
    ! aware of the name of the clock tracer?
    do i=1,num_tracer
        ! replace w test_trace_names?
        ! old functionality invoked with tracers_suite_flag
        name = get_tracer_name(i)
        ! for simple physics the order is :
        ! (a) sphum(1) + (b) tracers(num_tracer) + (c) aoa_tracers(4)
