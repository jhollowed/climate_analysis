...
# root should be on branch jhollowed/eam/cldera-clock-tracers
MY_E3SM_ROOT="/global/homes/j/jhollo/E3SM/E3SM_myFork_CLOCK"
MODEL="${MY_E3SM_ROOT}/cime/scripts/create_newcase"

CONFIGS="/global/homes/j/jhollo/repos/climate_analysis/CLDERA/AOA/cldera_clock_case_builder_E3SM/configs"
if [ -z "$CUSTOMNL" ]; then
    NL=${CONFIGS}/user_nl_eam_cldera_clock_trcs
else
    NL=$CUSTOMNL
fi

CASES="/global/homes/j/jhollo/repos/climate_analysis/CLDERA/AOA/cldera_clock_case_builder_E3SM/cases"
CASENAME="E3SM_${RES}_L72_${COMPSET}_clocks${SUFFIX}"
CASE=${CASES}/${CASENAME}
...
if [[ ! -d "$CASE"  ||  $BUILD_FLAG != "0" ]]; then
    ...
    ./xmlchange --append --file env_build.xml --id CAM_CONFIG_OPTS --val "-cldera_clock_trcs "
