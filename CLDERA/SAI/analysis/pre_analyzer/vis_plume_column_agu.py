...
if(main_figure):
    ...
    cf1 = ax1.contourf(t.to(u.year), zk.to(u.km), m_dens[2].T, cmap=cm, levels=clev, extend='both', vmin=-7.5)
    ax1.contour(t.to(u.day), zk.to(u.km), m_dens[2].T, levels=clev_thin_contours, colors=['k']*len(clev_contours), linewidths=0.5)
    ax1.set_xlim([0, 5])
    ax1.set_xlabel('$t$ [years]', fontsize=label_fs)
    claut.add_annotation_box(ax1, 'Sulfate', loc='upper right', fs=label_fs)
    ...
    ax4.plot(t.to(u.month), m_tot[0], 'r', label='SO$_2$', lw=4)
    ax4.plot(t.to(u.month), m_tot[2], 'orange', label='sulfate', lw=4, zorder=0)
    ax4.set_xlabel('$t$ [months]', fontsize=label_fs)
    ax4.set_ylabel('total mass [Mt]', fontsize=label_fs)
    ax4.legend(fancybox=False, fontsize=label_fs, loc='upper right')
    ax4.set_xlim([-0.5, 24])

    plt.tight_layout()

    # manually add contour labels after tight layout
    cc1 = ax1.contour(t.to(u.day), zk.to(u.km), m_dens[2].T, levels=clev_contours, colors=['k']*len(clev_contours))
    ax1.clabel(cc1, cc1.levels, inline=True, fmt='%.0f', fontsize=tick_fs, manual=((0.78, 21.7), (2.9, 22.3), (4.71, 23.2), (6.65, 23.4)))

    plt.savefig('../figs/sai_column_main_fig_agu.png', dpi=300)
...
if(heating_figure):
    ...
    plt.savefig('../figs/sai_column_heating_fig_agu.png', dpi=300)
