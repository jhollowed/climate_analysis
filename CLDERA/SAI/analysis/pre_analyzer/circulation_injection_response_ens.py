...
def analyze_response(run1, run2, run1_native=None, run2_native=None, overwrite=False,  
    ...
    t_windows = [slice(60+inj_delay, 90+inj_delay), slice(150+inj_delay, 180+inj_delay)]
    #t_labels = ['month 2\n', 'month 5\n']
    t_labels = ['', '']
    ...
    axTr.plot(time, SO21/1e9, '-r',  label='SO$_2$')
    axTr.plot(time, sulf1/1e9,'-', color='orange',label='sulfate')
