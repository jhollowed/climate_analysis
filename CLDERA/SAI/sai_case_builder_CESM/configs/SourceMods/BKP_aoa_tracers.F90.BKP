...
!===============================================================================
  ...
  subroutine aoa_tracers_timestep_tend(state, ptend, cflx, landfrac, dt)
    ...
    t= (day*24*60*60) + sec       ! assume eruption occurs at t=0
    plat = 15.15_r8 * deg2rad
    plon = 120.35_r8 * deg2rad
    pz = 25000.0_r8
    pzm  = 15000.0_r8               
    pxm = 200000.0_r8                
    ptau = 0.00002_r8                
    pA = 3000.0_r8                 
    pkSO2 = 1.0_r8/2592000.0_r8
    pkAsh  = 1.0_r8/345600.0_r8
    p00  = 100000.0_r8
       ...
       do i = 1, ncol
          ...
          ptend%q(i,k,ixaoa1) = -pkSO2 * state%q(i, k, ixaoa1) + &
                                pA * EXP(-(1.0_r8/2.0_r8) * &
                                         (((state%lat(i)-plat)/(pxm/(4.0_r8*rearth)))**2.0_r8 + &
                                          ((state%lon(i)-plon)/(pxm/(4.0_r8*rearth)))**2.0_r8)) * &
                                     EXP(-(1.0_r8/2.0_r8) * &
                                         ((state%zm(i,k)-pz)/(pzm/4.0_r8))**2.0_r8) * &
                                     EXP(-ptau * t)                 
          ! AOA2
          ! --JH--: Ash
          ...
          ! add logic for handling time profile
          
          ! HORZ
          ! --JH--: Potential Temperature
          ! initialize within the first minute of the injection
          if(t < 60.0_r8) then
              state%q(i,k,ixht) = state%t(i,k) * (p00 / state%pmid(i,k))**(rair/cpair)
          endif
